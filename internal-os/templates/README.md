# Templates

> Ready-to-use templates for client management, reporting, and communication.

## Template Index

| Template | Description | Format |
|----------|-------------|--------|
| [Client Tracker](./client-tracker.md) | Master spreadsheet for all client visibility | Google Sheets |
| [Monthly Report](./monthly-report.md) | Client performance report template | Google Slides |
| [Client Onboarding Checklist](./onboarding-checklist.md) | Checklist for new client setup | Google Sheets / ClickUp |
| [Campaign Brief](./campaign-brief.md) | Template for planning campaigns | Google Docs |

---

## How to Use Templates

1. **Copy the template** - Never edit the master, always make a copy
2. **Customize for client** - Adjust branding, fields as needed
3. **Keep templates updated** - If you improve one, update the master

---

## Template Locations

All master templates live in:
`Google Drive > Boring Ecom > Templates`

**Naming convention:** `[Template Name] - MASTER TEMPLATE`
